...
export default function Notes() {
    ...
    useEffect(() => {
        const fetchData = () => {
            getDocs(query(collection(db, user.uid), orderBy('date', 'desc')))
                .then((querySnapshot) => {
    ...
    const handleDelete = (id) => async () => {
        if (await deleteNote(id)) router.refresh();
    };
                    ...
                    {notes.length > 0 ? (
                        notes.map((note, i) => (
                            <Link
                                href={`/notes/${note.id}`}
                                passHref
                                className="rounded-lg p-6 bg-neutral-700 block max-w-full hover:bg-neutral-500"
                                key={i}
                            >
                                <h5 className="mb-2 text-xl font-bold leading-tight text-neutral-50">{note.title}</h5>
                                <p className="mb-4 font-light text-neutral-200 truncate">{note.content}</p>
